...
#
# node:24-slim (glibc) — safest base for @libsql/client native bindings.
# Single Fastify process serves /api/* and the built SPA out of /app/dist.
...
# Build-time provenance for the "about" popover / health version. `.git` is NOT in
# the build context (.dockerignore excludes it), so CI computes short SHA / tag /
# timestamp and forwards them as ARGs → env. tsup's `define` bakes them into the
# server bundle. Unset (a plain `docker build`) degrades to "unknown".
