...
export class RefreshService {
  ...
  /** Start a full refresh (background). Throws 409 if one is already running. */
  async startAll(): Promise<void> {
    if (this.isRunning) throw conflict('A refresh is already running', 'REFRESH_RUNNING');
    const rows = await this.store.listAllForRefresh();
    this.status = {
      state: 'running',
      current: 0,
      total: rows.length,
  ...
  /** Background hydration of freshly-created stubs (post-add). Never throws. */
